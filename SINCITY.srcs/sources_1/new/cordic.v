...
module cordic #(
    ...
    input  wire                     rst,
    input  wire                     start,
    input  wire signed [WIDTH-1:0]  angle,     // input angle (radians in Q2.14)
    output reg  signed [WIDTH-1:0]  cos_out,   // cosine result (Q2.14)
    output reg  signed [WIDTH-1:0]  sin_out,   // sine result (Q2.14)
    output reg                      done
);
    ...
    //  Internal registers
    // ------------------------------------------------------------------
    localparam signed [WIDTH-1:0] K_INIT = 16'd9957;   // 0.60725 * 16384
    ...
    // ------------------------------------------------------------------
    //  Angle lookup table (Q2.14)
    // ------------------------------------------------------------------
    initial begin
        atan_table[0]  = 16'd12868; // atan(1)
        atan_table[1]  = 16'd7596;  // atan(1/2)
        atan_table[2]  = 16'd4013;  // atan(1/4)
        atan_table[3]  = 16'd2037;  // atan(1/8)
        atan_table[4]  = 16'd1021;  // atan(1/16)
        atan_table[5]  = 16'd512;   // atan(1/32)
    ...
    // ------------------------------------------------------------------
    //  Barrel shifters for variable shifts
    // ------------------------------------------------------------------
    ...
    // x update:  if (z >= 0) x = x - (y>>i) else x = x + (y>>i)
    adder_subtractor #(.WIDTH(WIDTH)) add_x (
        .a(x),
        .b(y_shifted),
        .sub(z[WIDTH-1]==0 ? 1'b1 : 1'b0), // subtract when z>=0
        .result(x_next_addsub),
        .overflow(overflow_dummy1),
        .carry_out(carry_dummy1)
    );

    // y update:  if (z >= 0) y = y + (x>>i) else y = y - (x>>i)
    adder_subtractor #(.WIDTH(WIDTH)) add_y (
        .a(y),
        .b(x_shifted),
        .sub(z[WIDTH-1]==0 ? 1'b0 : 1'b1), // subtract when z<0
        .result(y_next_addsub),
        .overflow(overflow_dummy2),
        .carry_out(carry_dummy2)
    );

    // z update:  if (z >= 0) z = z - atan(i) else z = z + atan(i)
